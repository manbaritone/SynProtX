...
SAVER_LOGGER = logging.getLogger("SynProtXgcn.Saver")
MISC_LOGGER = logging.getLogger("SynProtXgcn.MISC")
...
def load_dataset(smiles, Y, ID):
    ...
    for idx, i in enumerate(ID):
        ...
        data = Data(x=x, edge_index=edge_index, y=Y[idx], ID=i)
        d.append(data)
...
class SynProtXgcn_predictor(nn.Module):
    def __init__(
        self,
        node_feat_size: int,
        out_channels: int,
        dropout_convolution_rate: float,
        ...
        print("dropout_rate:", dropout_convolution_rate)
        ...
        print("MLPu:", MLPu)
        print("dropout_gene:", dropout_gene)
        ...
        self.dropout = nn.Dropout(dropout_convolution_rate)
        # SMILES1 graph branch
        self.n_output = out_channels
        self.drug1_conv1 = GCNConv(node_feat_size, node_feat_size)
        self.drug1_conv2 = GCNConv(node_feat_size, node_feat_size * 2)
        self.drug1_conv3 = GCNConv(node_feat_size * 2, node_feat_size * 4)
        self.drug1_fc_g1 = torch.nn.Linear(node_feat_size * 4, node_feat_size * 2)
        self.drug1_fc_g2 = torch.nn.Linear(node_feat_size * 2, out_channels)

        # SMILES2 graph branch
        self.drug2_conv1 = GCNConv(node_feat_size, node_feat_size)
        self.drug2_conv2 = GCNConv(node_feat_size, node_feat_size * 2)
        self.drug2_conv3 = GCNConv(node_feat_size * 2, node_feat_size * 4)
        self.drug2_fc_g1 = torch.nn.Linear(node_feat_size * 4, node_feat_size * 2)
        self.drug2_fc_g2 = torch.nn.Linear(node_feat_size * 2, out_channels)
    ...
    def forward(self, data):
        ...
        if self.use_feamol:
            y_final = torch.cat((y_final, x1), 1)
            y_final = torch.cat((y_final, x2), 1)
        if self.use_feagene:
...
def model_tuning(
    ...
    dropout_convolution_rate = trial.suggest_float("dropout_convolution_rate", 0.1, 0.9)
    ...
    for fold in range(FOLDS):
        ...
        model = SynProtXgcn_predictor(
            node_feat_size=node_dimension,
            out_channels=output_units_num,
            dropout_convolution_rate=dropout_convolution_rate,
        ...
        # print('---- SynProtXgcn Model Architecture ----')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Model configuration for SynProtXgcn Model"
    )
    parser.add_argument(
        "-d",
        "--database",
    ...
    parser.add_argument(
        ...
        dest="resume_run_dir",
        default=None,
        help="if experiment folder exist, resume run",
    )
    parser.add_argument(
        "--no-feamol",
        dest="no_feamol",
        default=False,
        action="store_true",
        help="if added, disable GCN for molecule",
    )
    parser.add_argument(
        "--no-feagene",
        dest="no_feagene",
    ...
    dropout_convolution_rate = hyperparam.get("dropout_convolution_rate")
    ...
    if not os.path.exists(summaryfile):
        ...
        X.insert(0, "Molecule_GCN", np.nan)
